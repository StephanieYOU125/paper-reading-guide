...
function loadDB(){
  let db;
  try{db=JSON.parse(localStorage.getItem(DB_KEY)||'null');}catch{}
  if(!db||!Array.isArray(db.papers)){
    ...
    const first=normalizePaper(hasLegacy?legacy:blankPaper());
    db={papers:[first],activeId:first.id};
...
function persist(message='已自動儲存'){
  ...
  progress();renderLevelStatus();renderLibrary(document.getElementById('paperSearch')?.value||'');
}
...
function renderLevelStatus(){
  ...
  document.getElementById('currentLevelName').textContent=achieved?levels[achieved-1].name:'尚未建立基本理解';
...
function renderLibrary(query=''){
  ...
  const papers=[...db.papers].sort((a,b)=>new Date(b.updatedAt)-new Date(a.updatedAt)).filter(p=>!q||[p.paperTitle,p.paperAuthor,p.paperVenue].join(' ').toLowerCase().includes(q));
  ...
  box.querySelectorAll('.edit-paper').forEach(b=>b.onclick=()=>openPaper(b.dataset.id));
  box.querySelectorAll('.delete-paper').forEach(b=>b.onclick=()=>deletePaper(b.dataset.id));
}

function openPaper(id){
  persist();const p=db.papers.find(x=>x.id===id);if(!p)return;state=normalizePaper(p);db.activeId=state.id;localStorage.setItem(DB_KEY,JSON.stringify(db));fillEditor();document.getElementById('editorTop').scrollIntoView({behavior:'smooth'});
}
function deletePaper(id){
  const p=db.papers.find(x=>x.id===id);if(!p)return;if(!confirm(`刪除「${p.paperTitle||'未命名論文'}」？此動作無法復原。`))return;
  db.papers=db.papers.filter(x=>x.id!==id);
  if(!db.papers.length)db.papers=[blankPaper()];
  if(state.id===id){state=db.papers[0];db.activeId=state.id;fillEditor();}
  localStorage.setItem(DB_KEY,JSON.stringify(db));renderLibrary(document.getElementById('paperSearch')?.value||'');
}
function newPaper(){persist();state=blankPaper();db.papers.unshift(state);db.activeId=state.id;localStorage.setItem(DB_KEY,JSON.stringify(db));fillEditor();renderLibrary();document.getElementById('editorTop').scrollIntoView({behavior:'smooth'});}
...
function fillEditor(){
  ['paperTitle','paperAuthor','paperVenue','paperUrl','oneMinuteSummary'].forEach(id=>document.getElementById(id).value=state[id]||'');
  guideItems.forEach(x=>{document.getElementById('a-'+x.key).value=state.answers[x.key]||'';document.getElementById('c-'+x.key).checked=!!state.checks[x.key];});
  levels.forEach(l=>{l.checks.forEach((_,i)=>document.getElementById(`lc-${l.n}-${i}`).checked=!!state.levelChecks[l.n][i]);document.getElementById(`ln-${l.n}`).value=state.levelNotes[l.n]||'';});
  document.getElementById('editorTitle').textContent=state.paperTitle?`編輯：${state.paperTitle}`:'編輯這篇論文';
  progress();renderLevelStatus();updateLevelPanels();renderLibrary(document.getElementById('paperSearch')?.value||'');
}
...
document.addEventListener('DOMContentLoaded',()=>{
  renderGuide();renderLevelChecks();bind();theme();fillEditor();
  document.getElementById('paperSearch').oninput=e=>renderLibrary(e.target.value);
  document.getElementById('newPaperBtn').onclick=newPaper;
  document.getElementById('saveNowBtn').onclick=()=>persist('已儲存');
